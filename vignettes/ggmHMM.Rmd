...
As shown above, the default is to plot infered states for all individuals. In the plot below, the states for only a subset of individuals is plotted. We also use the function `scale_fill_manual()` from the `ggplot2` package to give new colors to the states as well as providing custom state labels.
```{r}
plot_viterbi(
  states = states, # dataframe with states
  ID = 1:5 # vector specifying subjects to plot. integer for a single subject is also possible
) +
  scale_fill_manual(values = c("red", "green"), # choose new 
                    labels = c("State One", "State Two"))
```

# Editing plot
The examples above already teased that plots returned by functions from the package can be easily customized. Each plotting function from `ggmHMM` returns an object of type `ggplot`. This means that functions from the `ggplot2` package can be used to customize plots, such as customizing themes.

...
#'
#' @param model Object of type `mHMMbayes::mHMM` or `mHMMbayes::mHMM_gamma`,
#' created using [mHMMbayes::mHMM()] or [mHMMbayes::obtain_gamma()].
#' @param level String specifying the level of transition distributions to plot.
#' Options are "group" and "subject".
#' @param subject Optional integer or integer vector specifying
#' the subject(s) to plot.
#' @param digits Integer specifying the number of digits to round to.
#' @param facet Logical specifying whether subjects should be faceted
#' when plotting subject-specific transition probability matrices.
#' @param ncol_facet Integer specifying the number of columns in the facet grid
#' when plotting subject-specific transition probability matrices.
#'
#' @return
  ...
  if (inherits(model, "mHMM")) {
    ...
    if (is.null(level) || !(level %in% c("group", "subject"))) {
      cli::cli_abort(
        c(
          ...
          "i" = "Please specify 'group' to plot the group-level parameters,
    ...
    if (inherits(gamma_matrix, "list")) {
      if (level == "group") {
        cli::cli_warn(
          c(
            "The specified level is {.val group},
            but you provided subject-specific transition probability matrices.",
            "i" = "Subject-specific transition probabilities will be plotted.",
            "i" = "To plot group level transition probabilities,
            provide an object of {.cls mHMMbayes::mHMM}
            or a group level transition probability matrix."
          )
        )
      }
      level <- "subject"
      ...
      if (level == "subject") {
        cli::cli_warn(
          c(
            "The specified level is {.val subject}, but you provided
            group-level transition probability matrices.",
            "i" = "Group-level transition probabilities will be plotted.",
            "i" = "To plot subject-specific transition probabilities,
            provide an object of {.cls mHMMbayes::mHMM}
            or subject-specific transition probability matrices."
          )
        )
      }
      level <- "group"
